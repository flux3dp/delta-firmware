...
class ReadAll(object):
    ...
    def read(self, timeout=None):
        ...
        while rlist:
            if timeout is not None and time() - start_at > timeout:
                raise RuntimeError("Timeout")

            ttl = min(3., time() - start_at)
            rl, wl, xl = select.select(rlist, (), (), ttl)
            for fd in rl:
                buf = fd.read()
                if buf:
                    self.__buffers[self.__rlist.index(fd)].write(
                        buf.decode("utf8"))
                else:
                    rlist.remove(fd)
...
if __name__ == "__main__":
    ...
    proc = subprocess.Popen(["ls", "/werqwerqwer"], stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    stdout, stderr = read_all([proc.stdout, proc.stderr])
    print("STDOUT: %s" % stdout)
    print("STDERR: %s" % stderr)
